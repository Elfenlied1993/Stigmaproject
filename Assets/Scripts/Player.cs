...
public class Player : MonoBehaviour
{
    ...
    public void Jump()
    {
        if (_moveState != MoveState.Jump)
        {
            ...
            {
                _rigidbody.velocity = (Vector3.up * JumpForce * Time.deltaTime + Vector3.right * JumpForce / 2 * Time.deltaTime);
    ...
    public void Crouch()
    {
        if (_moveState != MoveState.Jump)
        {
            if (_moveState != MoveState.Crouch)
            {
                _moveState = MoveState.Crouch;
                _animatorController.Play("Crouch");
            }
        }
